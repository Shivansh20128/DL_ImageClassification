...
But when I used my GPUs to accelerate the process, it took me only 20 minutes, with no priorities changed. Yes! That is the scale of impact we are talking about.
...
1. In the installation, it asks if you want to add Anaconda3 to the path variable and if you want to register Anaconda3 as default Python3.9. Check both the boxes (Even if they become red).
1. Leave the other settings as they are and install Anaconda on your system.


Now that you have Anaconda installed on your system, we are going to use it. But before, you need to make a folder in which you want to do your project and navigate to the folder in the command prompt/terminal.

At this point, we are going to use some terminal commands to configure some things.

1. First, we are going to make a virtual environment so that your system environment does not get affected by whatever we are going to do. This will make it easy for you to try out new things without worrying about your system's settings. Because in case you mess up, you can always delete the virtual environment and create a new one. To create a new virtual environment, enter the following command in the command prompt:
```
python -m venv myenv
```
This will create a new virtual environment named "myenv". you can give it any other name.

2. After that, we will activate the virtual environment. To do that, enter the command:
```
.\myenv\scripts\activate
```
This will activate the virtual environment. You should be able to see the name of your virtual environment at the start of the line now.

3. Now that your virtual environment is active, we will install "ipykernel" int it.
```
pip install ipykernel
```

4. Then attach the ipykernel to your virtual environment.
```
...
5. Now, you can open Jupyter lab with the command ```jupyter lab```and explore it a bit if you are new. Try to make a jupyter notebook in the new environment and run it (You can do that by changing the kernel).
...
1. Since we want to use GPUs in our development, we need to install an older version of TensorFlow because support has not been added yet for the newer versions. I am going to install TensorFlow 2.8 in my system.
```
...
1. The first thing we need to do is download Visual Studio 2019 from the Microsoft website. This step is required because Microsoft Visual C++ Build tools is one of the requirements for Tensorflow GPU acceleration. Here is the link where you can find it. https://visualstudio.microsoft.com/vs/older-downloads/
1. After downloading the file, start the installation. While installing, you will asked about the workloads you want to install in the Visual Studio. Check the boxes for "Python Development" under Web & Cloud, and "Desktop development with C++" under Desktop & Mobile. Although we don't need Python development right now, I still recommend installing it in case you decide to use Python development in the future.
1. Click the install button.


# Enable GPU Acceleration

Now, we are near the end of GPU acceleration. We need two pieces of software to be installed on our system to enable our GPUs to be detected by TensorFlow. These two software are CUDA and cuDNN. Both these software are available on the web.
But before proceeding, I want you to go to the link https://www.tensorflow.org/install/source_windows#gpu

Here, you will see what versions of each software we have installed. CUDA 11.2 and cuDNN 8.1 are the latest versions that are supported at the current time.
Let us install these one by one.

## Installing CUDA
1. Go to the following link. 
https://developer.nvidia.com/cuda-11.2.0-download-archive

2. Select your operating system, your architecture, your Windows version, and the installer type you want to download. Note that if you are on Windows 11, version 10, CUDA will still work for you, so don't worry about that. Download the installer and start installing.
3. Note that you may be asked to create an NVIDIA account first. So go ahead and create your account and install CUDA.
4. While installing CUDA, you don't need to change any settings and proceed with the default recommended installation.
...
# Checking GPUs
Now we can check if the tensorflow detects our GPUs.
